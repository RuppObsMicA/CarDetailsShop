...
const isResponseSignInFailure = (response: ResponseSignInSuccess | ResponseSignInFailure): response is ResponseSignInFailure => {
        ...
        mutate(data, {
            onSuccess: (responseData) => {
                if (responseData && !isResponseSignInFailure(responseData)) {
                    dispatch(authActions.login({token: responseData.token, role: responseData.role}));
                } else {
                    setNotificationMessage(responseData.message);
                }
            },
            onError: (error) => {
                setNotificationMessage("An error occurred during signIn. Please try again later.");
            }
        });
    };
